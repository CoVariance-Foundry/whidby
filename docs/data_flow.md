...
Bulk Explore data builds use `scripts/explore/bulk_score.py` as an operational runner over the same scoring and persistence path. The runner selects DataForSEO-ready metros with a rank-and-rent default ordering, caps `mega_5m_plus` markets only for the default strategy, calls `POST /api/niches/score` for each city/service pair, verifies `reports`, `metro_scores`, `metro_score_v2`, and `seo_facts` persistence before marking an attempt successful, and appends a JSONL audit row for every success, partial persistence failure, or API failure. Resume state merges report-backed `explore_market_cells` rows with legacy `metro_scores` + `reports` rows so interrupted runs skip all known cached reports rather than only one read model.
