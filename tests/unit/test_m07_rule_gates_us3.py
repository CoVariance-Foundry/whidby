...
_BALANCED_WEIGHTS = {
    "demand": 0.25, "organic_competition": 0.15,
    "local_competition": 0.20, "monetization": 0.20, "ai_resilience": 0.15,
}
...
def test_threshold_gate_caps_composite_when_component_below_5() -> None:
    score = compute_opportunity_score(
        component_scores={
            "demand": 4.0, "organic_competition": 80.0,
            "local_competition": 80.0, "monetization": 80.0, "ai_resilience": 80.0,
        },
...
def test_ai_floor_caps_composite_when_ai_resilience_is_low() -> None:
    score = compute_opportunity_score(
        component_scores={
            "demand": 90.0, "organic_competition": 90.0,
            "local_competition": 90.0, "monetization": 90.0, "ai_resilience": 10.0,
        },
        weights=_BALANCED_WEIGHTS,
    )
    assert score <= 50.0
