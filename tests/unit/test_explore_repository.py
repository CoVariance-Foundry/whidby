...
def test_list_city_rows_applies_service_demographic_and_state_filters() -> None:
    ...
    assert ("gt", "cbsa_code", "11111") in calls
    assert ("order", "population", {"ascending": True}) in calls


def test_list_city_rows_orders_desc_and_fetches_one_extra_row() -> None:
    client = FakeClient()
    repo = SupabaseExploreRepository(client)
    ...
    calls = client.tables["explore_market_cells"].calls
    assert ("order", "presentation_score", {"ascending": False}) in calls
    assert ("limit", 11) in calls
