...
| Repeated-state rule | Run three reports sequentially in one additional container. After each validated GET, wait five seconds, then capture cgroup `memory.current` and aggregate service-container process RSS by summing `VmRSS` for every numeric `/proc` PID; `memory.current <= 500000000` bytes and aggregate process RSS `<= 500000000` bytes, and neither run-three sample exceeds run one by more than `50000000` bytes. `memory.peak <= 500000000` bytes, and cache/tracker counts remain capped after every report. | Final benchmark uses `--sequential-runs 3 --quiescence-seconds 5 --max-retained-growth-bytes 50000000` plus bounded-state unit tests |
...
| Profile selection defaults | Generic/domain `ScoreRequest` and M5 helpers default to `full`; public `NicheScoreRequest` defaults to `interactive`; the customer BFF explicitly sends `collection_profile: "interactive"`. Explore refresh and other non-customer callers cannot inherit the bounded customer profile accidentally. | `tests/unit/test_collection_plan.py`, `tests/unit/test_pipeline_orchestrator.py`, `tests/unit/test_api_niches.py`, `apps/app/src/app/api/agent/scoring/route.test.ts` |
