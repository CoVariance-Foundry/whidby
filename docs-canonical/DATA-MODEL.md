...
- `metro_score_v2` stores persisted V2 score vectors, report lineage, benchmark confidence, product-facing `warning_codes`, and explanation facts for a city-service run. Explore and strategy read models prefer this table over legacy `metro_scores`, and `explore_market_cells` projects the same warning codes for cached-market surfaces.
