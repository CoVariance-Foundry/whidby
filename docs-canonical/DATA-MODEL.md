...
| OrganicCompetitorFact | Supabase `organic_competitor_facts` table | cbsa + niche + keyword + rank + date | Durable organic SERP competitor rows for Competitor Intel |
...
- `organic_competitor_facts` stores durable per-result organic evidence for Competitor Intel. Its grain is `(cbsa_code, niche_normalized, keyword, result_rank, snapshot_date)` and it preserves rank, title, domain, URL, DA, backlinks/referring-domain counts, Lighthouse score, schema/title-match signals, aggregator/local-business flags, source, and report lineage. Competitor Intel service-role reads must drop fact rows without visible report lineage rather than treating orphan facts as globally visible.
