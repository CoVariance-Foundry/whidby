...
`WHI-102` began on 2026-05-23. Production runtime preflight passed against Supabase project `eoajvifhbmqmoluiokcj`, and all six read-only preview buckets produced the expected 12 metro x 8 service pilot plan under ignored `reports/scoring_audit/preview_*.json`. The one-pair paid canary ran Waco, TX x roofing against `https://whidby-1.onrender.com`; the API returned HTTP 200 and report `69d8dccf-b1c9-453d-9ff7-7dffaf0c9850`, but the runner stopped the pilot as `partial_failure` because required persistence rows were missing from `metro_scores`, `metro_score_v2`, and `seo_facts`. Do not proceed to the full 12x8 paid pilot until that production persistence gap is fixed or explicitly explained.

The canary persistence gap was traced to target identity drift: the production API persisted legacy child rows, but under synthetic `cbsa_code=fallback:waco` instead of production CBSA `47380`, so V2/fact upserts could not satisfy production metro lineage. The local PR fix preserves explicit Supabase metro identity through `bulk_score.py`, `/api/niches/score`, `MarketService`, and `score_niche_for_metro` whenever the caller provides `cbsa_code`, `cbsa_name`, `population`, and a verified DataForSEO location code. Rerun only the one-pair canary after PR #78 is reviewed, merged, and deployed; the full paid pilot remains gated.
