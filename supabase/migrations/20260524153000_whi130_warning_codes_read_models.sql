...
latest_v2_scores AS (
    SELECT DISTINCT ON (v2.cbsa_code, v2.niche_normalized)
        ...
        coalesce(v2.warning_codes, ARRAY[]::text[]) AS warning_codes,
        v2.demand_strength,
...
score_union AS (
    SELECT
        ...
        v2.benchmark_confidence,
        CASE
            WHEN v2.cbsa_code IS NOT NULL THEN coalesce(v2.warning_codes, ARRAY[]::text[])
            ELSE coalesce(legacy.warning_codes, ARRAY[]::text[])
