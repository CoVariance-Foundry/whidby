...
function parseRunRequestBody(body: string): ExploreRefreshRunRequest {
  return JSON.parse(body) as ExploreRefreshRunRequest;
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    const upstream = await fetch(`${getApiBase()}/api/explore/refresh/runs`, {
      method: "POST",
    ...
    if (!upstream.ok) {
      return unavailableFromUpstream(
        upstream,
        "Explore refresh run service is unavailable.",
      );
    }

    const data = (await upstream.json()) as ExploreRefreshRunResponse;
    return NextResponse.json(data);
  } catch (err) {
    return NextResponse.json(
      {
        status: "unavailable",
        message: "Explore refresh run service is unavailable.",
