...
export type StrategyPathNodeDefinition = (typeof STRATEGY_PATH_REGISTRY)[number];
export type VisibleStrategyPathNode = Extract<
  StrategyPathNodeDefinition,
  { is_visible: true }
>;
export type VisibleStrategyPathId = VisibleStrategyPathNode["strategy_id"];
export type RunnableStrategyPathNode = Extract<
  StrategyPathNodeDefinition,
  { is_visible: true; is_runnable: true }
>;
export type RunnableStrategyPathId = RunnableStrategyPathNode["strategy_id"];
...
export function getVisibleStrategyPathNodes(): VisibleStrategyPathNode[] {
  return STRATEGY_PATH_REGISTRY.filter(
    (node): node is VisibleStrategyPathNode => node.is_visible,
  );
}

export function getRunnableStrategyPathNodes(): RunnableStrategyPathNode[] {
  return STRATEGY_PATH_REGISTRY.filter(
    (node): node is RunnableStrategyPathNode => node.is_visible && node.is_runnable,
  );
}
...
export function isUserFacingStrategyId(strategyId: string): strategyId is VisibleStrategyPathId {
  return Boolean(getStrategyPathNode(strategyId)?.is_visible);
}

export function isRunnableStrategyId(strategyId: string): strategyId is RunnableStrategyPathId {
